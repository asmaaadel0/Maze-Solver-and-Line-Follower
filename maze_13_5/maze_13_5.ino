...
long speed = 100; // 110;
int rot_speed = 100;
long replay_speed = 150;
long rot_replay_speed = 110;
...
void movecar2()
{
    ...
    if (pos == B11111 && LL && RR && RRR)
    {
        ...
        if (pos == B11111 && LL && RR && RRR)
        {
            // try to rotate to right to check if it reads black line or not
            SpeedLogic(0, 20);
            delay(100);
            if (pos == B11111 && LL && RR && RRR)
            {
...
void replay()
{
    ...
    if (readString[i] == 'L' && (LL || LL_first_array))
    {
        SpeedLogic(0, 0);
        delay(500);
        rotateLeft();
    ...
    else if (readString[i] == 'R' && RR) // right rear far sensor & the 5 sensors are white
    {
        SpeedLogic(0, 0);
        delay(500);
        rotateRight();
